...
## 12. Starting Skills — "Choose one" blocks aren't machine-readable

- **What:** Each class's **Starting Skills** entry mixes fixed grants and one or
  more "Choose one of the following" specialty blocks, but the structure is
  carried only by prose conventions that vary line to line, so the export
  flattens it into an ambiguous string list. Specific irregularities:
  - **Block headers vary:** "Choose one of the following:", "Choose One of the
    Following:", "Choose one of the below:" (case/word drift).
  - **Header-vs-inline inconsistency:** Druid's "Budding Wisdom" puts the header
    on its own line then options on following lines; Artisan's "Productive
    Equipment" / "The Land Provides" put the whole choice inline on one line
    (`Choose a Gathering Skill: Forage I, Prospect I, or Scavenge I (3)`).
  - **Embedded mini-choices in otherwise-fixed lines:** Druid's "Natural
    Survival" ends `…and either Forage I or Scavenge I`; Mage's "Dutiful
    Scholar" contains `Choose a Lore Skill`. These are real player choices
    buried inside a fixed-grant line.
  - **Within-option "or":** Fighter's `Short Weapons (3), Advanced Shields (2)
    or Two Weapon Style (2)` — the "or" splits one option into two, but reads
    like a skill list.
  - **Bracketed parameters mixed with skill names:** Artisan's `Choose one
    Apprentice Crafting: [Alchemy, (Ritual Magic AND Ritual Lore), Enchanting,
    or Tinkering]` — the bracket is a parameter on the skill "Apprentice
    Crafting", and "(Ritual Magic AND Ritual Lore)" is two skills bundled as one
    parameter.
  - **Variant skill spellings:** `Lore: Nature` / `Lore [Shadow]`,
    `Two-Weapon Style` (vs canonical "Two Weapon Style"), `Scavenge` (vs
    "Scavenge I"), `Basic Lock Skill` / `Basic Trap Skill` (vs "Basic Locks" /
    "Basic Traps"), `Bits & Pieces` (vs "Bits and Pieces"). These don't match
    the skills' canonical names.
- **Where:** Every class's `Starting Skills` H2 (Artisan is the worst case with
  three blocks and embedded brackets).
- **Parser workaround:** The starting-choice dropdowns are driven by a
  hand-curated `STARTING_CHOICES_CONFIG` in `src/data/starting-choices.js`,
  transcribed from this prose. A build-time test (`scripts/test.mjs`) fingerprints
  the skills each class's prose references and asserts the curated config covers
  exactly those, so a doc change fails loudly rather than silently drifting — but
  the structure itself still has to be maintained by hand.
- **Wanted edit:** Give the choice blocks consistent, parseable structure:
  - One canonical header phrase (e.g. always "Choose one of the following:")
    on its own line, with each option as a sibling list item beneath it.
  - Model embedded picks ("either Forage I or Scavenge I", "Choose a Lore
    Skill") as their own named choice block, not buried inside a fixed line.
  - Make each option a single list item; if an option is "A or B", make A and B
    two separate options instead.
  - Use canonical skill names (matching the Skills section) — no `Lore: X` /
    `Lore [X]` / hyphenation / `& ` ampersand variants.
  With that, `STARTING_CHOICES_CONFIG` could be generated by the parser the same
  way `startingSkills`/`progression` already are, eliminating the curated table.
