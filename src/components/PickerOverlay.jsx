...
import {
  availableFacets as computeAvailableFacets,
  passesFacets as facetsPass,
  passesFacetsExcept as facetsPassExcept,
  activeFacetCount as countActiveFacets,
  toggleFacetValue,
} from "./browse/facets.js";
...
export default function PickerOverlay({ spec, character, onClose }) {
  ...
  const allAxes = useMemo(
    () => [
      ...SURFACE_AXES.filter((a) => a.id !== "cost" || hasCost),
      {
        ...
        label: "Tags",
        multi: true,
        keys: (c) => entityOf.get(c.name)?.tags || [],
  ...
  const availableAxes = useMemo(() => allAxes.filter((a) => axisApplies(a, candidates)), [allAxes, candidates]);
  ...
  const isParameterized = (!isFollowing && !!readingEntity?.parameter) || paramOptions.length > 0 || paramTypeable;
  const commitName =
    isParameterized && paramValue
        ...
        <div className="b-picker-browse">
          ...
          {filtersOpen && (
            <FacetPopover
              ...
              entityOf={entityOf}
              passesExcept={passesFacetsExcept}
                    ...
                    return (
                      <li key={c.name}>
                        ...
                        >
                          ...
                          {spellTierKey(c) && (
                            <span className={`b-picker-row-tier b-tier-${spellTierKey(c)}`}>{spellTierLabel(c)}</span>
                          )}
                          ...
                          {typeof c.cost === "number" && c.cost > 0 && (
                            <span className="b-picker-row-cost">{c.cost} BP</span>
                          )}
                          {typeof c.cost === "string" && /^var/i.test(c.cost) && (
                            <span className="b-picker-row-cost">Var BP</span>
                          )}
                          {typeof c.bp === "number" && c.bp > 0 && (
                            <span className="b-picker-row-cost is-award">+{c.bp} BP</span>
                          )}
          ...
          {readingEntity ? (
            <>
              <header className="b-read-head">
                {isFollowing && (
                  <button className="b-detail-back" onClick={readBack}>
                    ‹ back to {selected}
                  </button>
                )}
                ...
                <p className="b-detail-type">
                  {readingEntity.type}
                  {readingEntity.tier ? ` · ${readingEntity.tier}` : ""}
                </p>
              </header>
              ...
              {!isFollowing && (
                <footer className="b-read-foot">
                  ...
                  {selectedLocked && (
                    <p className="b-read-warn">
                      Prereqs not met — you can still choose it, but the build won't be legal.
                    </p>
                  )}
                  <button
                    className="b-read-choose"
                    disabled={selectedTaken || needsParam}
                    onClick={() => onChoose(commitName)}
                  >
                    {selectedTaken ? "Already chosen" : needsParam ? "Pick an option above" : `Choose ${commitName}`}
                  </button>
                </footer>
              )}
            </>
  ...
  return (
    ...
    >
      <div className="b-facet-pop-head">
        <span>Filter by</span>
        {anyActive && (
          <button className="b-facet-clear" onClick={onClear}>
            clear all
          </button>
        )}
      </div>
              ...
              {f.options.map(([v]) => {
                ...
                return (
                  <label key={v} className={`b-facet-opt ${on ? "is-on" : ""} ${n === 0 && !on ? "is-zero" : ""}`}>
                    <input type="checkbox" checked={on} disabled={n === 0 && !on} onChange={() => onToggle(f.id, v)} />
                    {v}
                    <span className="b-facet-c">{n}</span>
                  </label>
                );
              })}
            </div>
