...
export const STANDARD_RESOURCES = [...resourcesJson.map((r) => normalizeResourceName(r.name)), "Wealth"];
...
export function IngredientKind({ name, onJump }) {
  ...
  return (
    <span className="b-ing-kind is-raw" title="Raw material — must be gathered">
      🌿 Raw
    </span>
  );
}
    ...
    <div className="b-craft-tree-node" style={{ marginLeft: depth ? 18 : 0 }}>
      <div className={`b-craft-tree-row ${cls}`}>
        ...
        {craftable ? (
          <button
            type="button"
            className="b-craft-tree-name is-link"
        ...
        <span className="b-craft-tree-tag">
          {node.kind === "have"
            ? "in inventory"
            : node.kind === "crafted"
              ? `craft${node.have > 0 ? ` (${node.have} on hand)` : ""}`
