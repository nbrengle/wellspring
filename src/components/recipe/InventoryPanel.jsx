...
export default function InventoryPanel({
  inventory, inventoryGroups, customItemInput, setCustomItemInput,
  onQtyChange, onQtySet, onDeleteItem, onAddCustomItem,
}) {
    ...
    return (
      <div key={name} className={`b-inventory-item ${qty > 0 ? "is-owned" : ""} ${kind === "crafted" ? "is-crafted" : kind === "custom" ? "is-custom" : ""}`}>
        <div className="b-inv-info">
          ...
          {deletable && (
            <button className="b-inv-delete" onClick={() => onDeleteItem(name)} title="Remove from inventory">✕</button>
          )}
        </div>
        <div className="b-inv-control">
          <button className="b-inv-btn" onClick={() => onQtyChange(name, -1)} disabled={qty === 0}>-</button>
          <input type="number" min="0" className="b-inv-input" value={qty} onChange={(e) => onQtySet(name, e.target.value)} />
          <button className="b-inv-btn" onClick={() => onQtyChange(name, 1)}>+</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="b-recipes-inventory">
      ...
      <form className="b-inventory-custom-form" onSubmit={onAddCustomItem}>
        ...
        />
        <button type="submit" className="b-combobox-toggle b-custom-add-btn">+</button>
      </form>
    </div>
  );
}
