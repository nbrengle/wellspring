...
export default function LineageGallery({ onPick }) {
  ...
  const cards = useMemo(() => {
    ...
    const filtered = q
      ...
      : list;
    filtered.sort((a, b) => sort === "options"
  ...
  return (
    <div className="b-lin-gallery">
      <div className="b-lin-gallery-controls">
        <input className="b-lin-search" type="search" placeholder="Search lineages by name, lore, or sublineage…"
               value={query} onChange={(e) => setQuery(e.target.value)} aria-label="Search lineages" autoFocus />
        <label className="b-lin-sort">Sort
      ...
      <div className="b-lin-gallery-grid">
        {cards.map(({ name, lin, tagline, nChallenges, nAdvantages }) => (
          <button key={name} className="b-lin-gallery-card" onClick={() => onPick(name)}>
            ...
            <span className="b-lin-gallery-counts">{nChallenges} challenges · {nAdvantages} advantages</span>
          </button>
        ))}
