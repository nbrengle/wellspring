...
export function useLineageHandlers({ setCharacter }) {
  const handleSetLineage = useCallback(
    (name) => {
      setCharacter((c) =>
        name === c.lineage
          ? c
  ...
  const handleSetSublineage = useCallback(
    (sub) => {
      setCharacter((c) => {
  ...
  const handleToggleLineageItem = useCallback(
    (field, name) => {
      setCharacter((c) => {
        const cur = c[field] || [];
  ...
  const handleSetLineageRep = useCallback(
    (field, baseName, rep) => {
      setCharacter((c) => {
        const cur = c[field] || [];
  ...
  const handleSetAdvantageChoice = useCallback(
    (advantage, value) => {
      setCharacter((c) => ({
        ...c,
