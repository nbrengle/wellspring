...
export const POOLS: PoolDef[] = [
  {
    id: "healing-touch",
    name: "Healing Touch Pool",
    definedBy: "Healing Touch",
    aliases: /healing touch pool|healing pool/i,
  }, // "healing pool" = Greater Healing Touch variant
  { id: "life-tap", name: "Life Tap Pool", definedBy: "Life Tap", aliases: /life tap pool/i },
  { id: "living-iron", name: "Living Iron Pool", definedBy: "Living Iron", aliases: /living iron pool/i },
  { id: "balance", name: "Balance Pool", definedBy: "The Balance of Life", aliases: /balance pool/i },
  {
    id: "maintenance",
...
const textOf = (e: any): string => [e?.description, e?.effect, e?.call].filter(Boolean).join("  ");
...
const PERMANENT_AUGMENT =
  /(additional|extra)\s+points?\s+(to|in)[^.]*\bper\b[^.]*level|increase[^.]*\b(maximum|max)\b[^.]*pool|\bper\b[^.]*level[^.]*pool/i;
...
export function poolRelation(entity: any, poolId: string): PoolRelation | null {
  ...
  if (
    /\b(expend|subtract|spend(s|ing)?|lose|draw|use|point[s]? from)\b[^.]*pool|from (the|their|its)[^.]*pool/i.test(
      text,
    )
  ) {
    return "spends";
...
export function characterPools(owned: OwnedLike[], classLevelOf: (className: string) => number): CharacterPool[] {
  ...
  const resolved = owned.map((o) => ({
    ...
    ent: lookupEntity(`classes:${o.name}`) ||
      ...
      lookupEntity(`powers:${o.name}`) || { name: o.name },
  }));
...
export function unresolvedPoolMentions(
  ...
  for (const p of POOLS) {
    ...
    if (!def) {
      out.push({ pool: p.id, reason: "missing-definer" });
      continue;
    }
