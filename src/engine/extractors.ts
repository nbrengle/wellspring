import type { Effect, Entity, CharacterState } from "./types.js";
...
function extractDiscounts(ent: any, character: CharacterState, id: string): Effect[] {
...
function extractGlobalBestows(ent: any, character: CharacterState, id: string): Effect[] {
...
function extractWealth(ent: any, _character: CharacterState, _id: string): Effect[] {
...
function extractStatMods(ent: any, _character: CharacterState, _id: string): Effect[] {
  if (ent && "statMods" in ent && Array.isArray(ent.statMods)) {
    return ent.statMods.map((mod: any) => ({ type: "STAT", stat: mod.stat, amount: mod.amount }));
  }
  return [];
}
...
const optBestows = (o: any) => o?.bestows || [];

function extractChooseOne(ent: any, character: CharacterState, _id: string): Effect[] {
  if (ent?.chooseOne?.kind === "build") {
    ...
    if (chosen) {
      ...
      const opt = ent.chooseOne.options.find((o: any) => o.text === chosen || optBestows(o).includes(chosen));
...
function extractLineageChoiceSpec(ent: any, character: CharacterState, _id: string): Effect[] {
  if (ent?.type === "advantage" || ent?.type === "challenge") {
    const spec = lineageChoiceSpec(ent as any);
...
function extractPowerSpellChoiceSpec(ent: any, character: CharacterState, _id: string): Effect[] {
...
function extractStudiedFocus(ent: any, character: CharacterState, _id: string): Effect[] {
...
function extractLevelDiscounts(ent: any, character: CharacterState, id: string): Effect[] {
  ...
  for (const c of charClasses) {
    ...
    const offers = ["innate", "utility", "basic", "advanced", "veteran", "classSkills", "rightHandPowers"].some((cat) =>
      (clsDef as any)[cat]?.some((p: any) => (p.id || p.name) === ent.name || p.id === id || p.name === ent.name),
    );
