...
const eligibleForms = matchers.filter((m) => m.type === "skills" || m.type === "perks" || m.type === "powers");
...
for (const c of classes) {
  POWER_TIERS.forEach((tier) => {
    (c[tier] || []).forEach((p) => {
      ...
      let prereqText = p.prerequisites || p.prereq;
      if (typeof prereqText === "string") {
        prereqText = prereqText.replace(/\s+(?:Innate|Utility|Basic|Advanced|Veteran|Class|Greater)?\s*Power/gi, "");
      }
      prereqs[id] = parsePrereq(prereqText, eligibleForms);
      const depIds = [...prereqs[id].skills, ...prereqs[id].anyOf.flat()];
      for (const dep of depIds) {
        (unlocks[dep] = unlocks[dep] || []).push(id);
      }
