...
const base = () => ({ classes: [{ name: 'Fighter', level: 6 }], skills: [], perks: [], powers: [], spells: [], flaws: [], devotions: [] });
const mage = () => ({ classes: [{ name: 'Mage', level: 6 }], skills: [], perks: [], powers: [], spells: [], flaws: [], devotions: [] });
...
test('cls opt sets the granting class for a slot power (multiclass)', () => {
  const mc = { classes: [{ name: 'Fighter', level: 3 }, { name: 'Rogue', level: 3 }], skills: [], perks: [], powers: [], spells: [], flaws: [], devotions: [] };
...
test('makeChar seeds the class starting kit (realistic character)', () => {
  ...
  ok(c.skills.every((s) => s.source.type === 'starting'), 'all seeded skills are starting-sourced');
});
test('makeChar add[] funnels through the real add API', () => {
  const c = makeChar('Fighter 4', { add: ['Battlemind', 'Athletics'] });
  ok(c.powers.some((p) => p.entityId === 'Battlemind' && p.costField === 'basicPowers'), 'power added + derived');
  ok(c.skills.some((s) => s.entityId === 'Athletics' && isPurchased(s.source)), 'purchased skill added');
});
