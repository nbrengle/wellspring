...
test("validation: take-once cap on purchases (generic OVER_CAP, e.g. Weapon Spec)", () => {
  ...
  const multiple = makeChar("Fighter 4", {
    archetypeName: "x",
    add: ["Weapon Specialization (Swords)", "Weapon Specialization (Daggers)"],
  });
  const rMultiple = validate(multiple);
  ok(
    rMultiple.prereqs.issues.some((i) => /Weapon Specialization/.test(i.item) && /taken once/i.test(i.text)),
...
test("validation: Advanced Classes limits and rules", () => {
  const legalAdv = {
    ...
    classes: [
      ...
      { name: "Shadowblade", level: 5 },
    ],
  };
  const rLegal = validate(legalAdv);
  const shadowbladeIssues = rLegal.prereqs.issues.filter(
    (i) => i.item === "Shadowblade" || i.item === "Advanced Classes",
  );
  ...
  const lowBase = {
    ...
    classes: [
      ...
      { name: "Shadowblade", level: 1 },
    ],
  };
  const rLowBase = validate(lowBase);
  ok(
    rLowBase.prereqs.issues.some(
      (i) => i.item === "Advanced Classes" && i.text.includes("until total level 10 has been reached"),
    ),
    "Advanced class blocked if base classes level < 10",
  );
  ...
  const tooHighLevel = {
    ...
    classes: [
      { name: "Fighter", level: 10 },
      { name: "Shadowblade", level: 6 },
    ],
  };
  const rTooHighLevel = validate(tooHighLevel);
  ok(
    rTooHighLevel.prereqs.issues.some((i) => i.item === "Shadowblade" && i.text.includes("maximum of 5 levels")),
    "Advanced class capped at 5 levels",
  );
  ...
  const tooManyAdv = {
    ...
    classes: [
      { name: "Fighter", level: 10 },
      { name: "Shadowblade", level: 1 },
      { name: "Spellbinder", level: 1 },
      { name: "Archmage", level: 1 },
    ],
  };
  const rTooManyAdv = validate(tooManyAdv);
  ok(
    rTooManyAdv.prereqs.issues.some((i) => i.item === "Advanced Classes" && i.text.includes("maximum of two")),
    "Max of two Advanced Classes",
  );
});
...
test("validation: Draconic Heritage character creation note", () => {
  ...
  const r = validate(c);
  ok(
    r.prereqs.notes.some(
      (n) => n.item === "Draconic Heritage" && n.text.includes("Must be taken at Character Creation"),
    ),
    "Heritage note is registered",
  );
});
...
test("every entity referenced by a rules relation resolves (no dangling refs)", () => {
  ...
  const add = (id) => {
    if (typeof id === "string" && id.includes(":")) referenced.add(id);
  };
