...
import {
  ...
  poolsReferenced,
  poolRelation,
  unresolvedPoolMentions,
  characterPools,
} from "../../src/engine/pool-registry.js";
...
test("pool-registry: known pools are catalogued with a definer", () => {
  for (const id of ["healing-touch", "life-tap", "living-iron", "balance", "maintenance"]) {
    ...
    ok(
      lookupEntity(`classes:${p.definedBy}`) || lookupEntity(`domains:${p.definedBy}`),
      `definer exists: ${p.definedBy}`,
    );
  }
});
...
test("pool-registry: tolerant matching resolves variant spellings (pre-source-fix)", () => {
  ...
  eq(
    poolRelation(ght, "healing-touch"),
    "augments-max",
    'Greater Healing Touch → permanent max boost (resolved via "healing pool" alias)',
  );
});
...
test("pool-registry: relation distinguishes PERMANENT max-boost from TEMPORARY refill", () => {
  ...
  eq(
    poolRelation(lookupEntity("classes:Greater Healing Touch"), "healing-touch"),
    "augments-max",
...
test("pool-registry: build guard surfaces pools with no derivable size", () => {
  ...
  ok(
    offenders.some((o) => o.pool === "life-tap" && o.reason === "no-formula"),
