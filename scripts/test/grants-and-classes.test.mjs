...
test("loadArchetype: every archetype loads with its class set", () => {
  for (const a of ARCHETYPES) {
    ...
    ok(
      classes.every((x) => x.name && x.level > 0),
      `${a.archetypeName || a.name}: class has name + level`,
    );
...
test("node.param is structured (parsed once), incl. on grants", () => {
  ...
  eq(
    lore
      ...
      .join(","),
    "Arcane,Religion",
    "purchased Lore params are structured",
  );
...
test("a stored innate power is materialized once, not double-counted", () => {
  ...
  const items = resolveCharacterGraph(makeChar("Socialite 4", { add: ["Practiced Manner"] })).filter((i) =>
    /^Practiced Manner$/.test(i.name),
  );
...
test("Agile Learner trades require owning the skill and are capped by its rank", () => {
  const base = (skills, ranks, trades) =>
    computeSlots({
      classes: [{ name: "Fighter", level: 4 }],
      skills: skills.map((s, i) => ({ entityId: s, source: Source.purchased(), ranks: ranks[i] || 1 })),
      agileLearnerTrades: trades,
    });
  ...
  eq(basic(r), 1, "owned: basic -1");
  eq(advanced(r), 1, "owned: advanced +1");
  // Doesn't own it: no trade.
  r = base([], [], { Fighter: 1 });
  eq(basic(r), 2, "unowned: basic unchanged");
  eq(advanced(r), 0, "unowned: advanced unchanged");
  // Owns 1, requests 3: clamped to capacity 1.
  eq(
    agileLearnerCapacity({ skills: [{ entityId: "Agile Learner", source: Source.purchased(), ranks: 1 }] }),
    1,
    "capacity = owned ranks",
  );
...
test("every class has parsed multiclassGrants that resolve to entities", () => {
  ...
  for (const name of Object.keys(CLASSES)) {
    ...
    for (const g of grants) {
      ok(
        lookupEntity(`skills:${cleanName(g.name)}`) || lookupEntity(`perks:${cleanName(g.name)}`),
...
test("Cleric spell-slots grow with level (L4 4/0/0 → L6 6/2/0)", () => {
  ...
  eq(ss(4).novice, 4, "L4 novice");
  eq(ss(4).adept, 0, "L4 adept");
  eq(ss(6).novice, 6, "L6 novice");
  eq(ss(6).adept, 2, "L6 adept");
});
test("spells-known picker offers novice + adept (all learnable tiers)", () => {
  ...
  ok(tiers.has("noviceSpells"), "novice offered");
  ok(tiers.has("adeptSpells"), "adept offered");
});
