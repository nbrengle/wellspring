...
for (const a of ARCHETYPES) {
  test(`archetype "${a.name}" is evaluated`, () => {
    ...
    if (a.name !== "Mystic Artisan" && a.name !== "Crafter Artisan") {
      ...
      ok(
        r.valid,
        `should be legal (flags: ${JSON.stringify({ over: r.overBudget, slots: r.slotsOver, prereq: r.prereqs.issues.length, below: r.belowFloor })})`,
      );
    }
  });
}
...
test("crafting capability: owned skill unlocks its discipline; tiers nest", () => {
  ...
  ok(
    al.recipes.every((r) => r.tier === "Apprentice"),
    "only apprentice recipes",
  );
  ...
  ok(
    tk.recipes.some((r) => r.tier === "Apprentice") && tk.recipes.some((r) => r.tier === "Greater"),
    "nests lower tiers",
  );
...
test("Basic Arcane / Basic Faith spell pools: sphere-gated, non-casters get any base class", () => {
  ...
  ok(
    mage.every((s) => fAr.includes(s)),
    "Mage pool ⊆ all-Arcane pool",
  );
  // sorted + de-duped
...
test("innate bonus cantrip (Cancel) is granted+locked, not a choosable slot", () => {
  const cant = (cls, lvl) =>
    computeSlots({ archetypeName: "x", classes: [{ name: cls, level: lvl }] }).find((s) => s.category === "cantrips");
...
test("a granted cantrip in the pick list does not double-count", () => {
  ...
  const r = computeSlots({
    archetypeName: "x",
    classes: [{ name: "Mage", level: 2 }],
    spells: [
      { entityId: "Cancel", source: Source.class("Mage") },
      { entityId: "Force Shield", source: Source.class("Mage") },
    ],
...
test("Fighter 2 / Rogue 2 yields separate per-class slot rows at L2 caps", () => {
  ...
  eq(fu.allowed, 1, "Fighter L2 utility");
  eq(ru.allowed, 1, "Rogue L2 utility");
});
