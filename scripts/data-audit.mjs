...
for (const c of read("classes.json")) {
  for (const arr of Object.values(c))
    if (Array.isArray(arr)) for (const p of arr) if (p && p.name) POWER_NAMES.add(p.name);
}
...
function* entities() {
  for (const c of read("classes.json")) {
    for (const [tier, arr] of Object.entries(c)) {
      ...
      for (const p of arr)
        if (p && p.name)
          yield { file: "classes", type: `${c.name}/${tier}`, name: p.name, desc: p.description || "", obj: p };
    }
  }
  const flat = {
    "skills.json": "description",
    "perks.json": "description",
    "flaws.json": "description",
    "domains.json": null,
    "devotions.json": "lore",
    "accents.json": "description",
    "conditions.json": "description",
    "effects.json": "description",
    "modifiers.json": "description",
    "defense-calls.json": "description",
    "resources.json": "description",
    "types.json": "description",
    "crafting-concepts.json": "description",
    "ritual-concepts.json": "description",
    "crafting-recipes.json": "description",
    "ritual-recipes.json": "effect",
    "glossary.json": "definition",
  };
...
const CHECKS = [
  ...
  (e) =>
    ...
    !/\b(here|for reference|below|presented|shown|as follows)\s*[:：]\s*$/i.test(e.desc.trim())
      ? 'ends with ":" (dropped list?)'
      : null,
  // PROMISES MORE — "following"/"below"/"at various levels" but text looks cut off.
  (e) =>
    /\b(the following|listed below|as follows|at various .* levels)\s*[:.]?\s*$/i.test(e.desc.trim())
      ? "promises a list that isn't there"
      : null,
  // STAT-TABLE GARBAGE — body is mostly bare numbers (grabbed a progression table).
  (e) => {
    const toks = e.desc.trim().split(/\s+/);
    if (toks.length >= 6 && toks.filter((t) => /^[\d-]+$/.test(t)).length / toks.length > 0.4)
      return "looks like a stat-table fragment";
    return null;
  },
  ...
  (e) =>
    e.desc.trim().length > 0 && e.desc.trim().length < 15 && /classes/.test(e.file)
      ? `very short (${e.desc.trim().length} chars)`
      : null,
];
...
for (const e of entities()) {
  for (const check of CHECKS) {
    ...
    if (reason) {
      findings.push({ ...e, reason, more: megadocHasMore(e) });
      break;
    }
  }
}
...
const sev = (r) => {
  const i = order.findIndex((o) => r.startsWith(o));
  return i < 0 ? 99 : i;
};
...
for (const f of findings) {
  ...
  if (group !== lastReasonGroup) {
    console.log(`\n── ${group} ──`);
    lastReasonGroup = group;
  }
