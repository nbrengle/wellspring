...
const descOf = (e, typePrefix) =>
  e?.description || e?.desc || (e?.name && typePrefix && lookupEntity(`${typePrefix}:${e.name}`)?.description) || "";
...
const SIGNALS = [
  {
    cat: "maxLifePoints",
    re: /\bMaximum Life Points?\b|\bLife Points?\b[^.]*\bmax|max[^.]*\bLife Points?\b|\+\s*\d+\s*(?:Base\s+)?(?:Maximum\s+)?Life Points?/i,
  },
  // Maximum Spikes only — NOT "Spike Damage" (a damage bonus, different stat).
  {
    cat: "maxSpikes",
    re: /(?:Maximum|Base Maximum)\s+Spikes?\b(?!\s+Damage)|\+\s*\d+\s+(?:Bonus\s+)?Maximum\s+Spikes?\b/i,
  },
  { cat: "armor", re: /\bNatural Armor\b|\+\s*\d+\s*(?:physical\s+)?Armor Points?\b|\bArmor Points?\b[^.]*\bmax/i },
  {
    cat: "slots",
    re: /\badditional\s+(?:Cantrip|Utility|Basic|Advanced|Veteran)\s+(?:Tier\s+)?Power|\badds?\s+\d+\s+to\s+the\s+number\s+of\s+Known Spells|additional\s+Known Spell/i,
  },
  ...
  {
    cat: "spellSlot",
    re: /\badditional\s+(?:Novice|Adept|Greater|highest[- ]level)\s+spell-?slot|gains?\s+an?\s+additional\s+spell-?slot/i,
  },
  {
    cat: "wealth",
    re: /\bWealth\b[^.]*\b(?:beginning of (?:each|every) (?:game|event)|per Event)|Alternatively,?\s*\d+\s*Wealth|\d+\s*Wealth\s+at the beginning of (?:their\s+)?first/i,
  },
  {
    cat: "grant",
    re: /\bgains?\s+(?:the\s+)?[A-Z][\w' -]+\s+(?:Perk|Power|Skill|Cantrip|Spell)\b|\bgrants?\s+(?:them|themselves|the character)\b/i,
  },
...
const SPELLSLOT_RE =
  /\badditional\s+(Novice|Adept|Greater|highest[- ]level)\s+spell-?slot|gains?\s+an?\s+additional\s+spell-?slot/i;
const SPELLSLOT_FIELDS_SCANNED = ["skill", "perk", "lineage"];

function builderCatches(cat, text, kind) {
  switch (cat) {
    case "maxLifePoints":
      return anyMatch(STAT_RE.lifePoints, text);
    case "maxSpikes":
      return anyMatch(STAT_RE.spikes, text);
    case "armor":
      return anyMatch(STAT_RE.armor, text);
    case "slots":
      return anyMatch(SLOT_RE, text);
    case "wealth":
      return anyMatch(WEALTH_RE, text);
    case "grant":
      return "graph";
    // innateBonusCantrips (PR #12) extracts the named cantrip from this prose.
    case "innateCantrip":
      return /\binnate\s+bonus\s+cantrip:\s*([^]*)/i.test(text);
    case "spellSlot": {
...
function* abilities() {
  ...
  for (const [cls, prog] of Object.entries(CLASS_PROGRESSION)) {
    for (const [lvl, row] of Object.entries(prog)) {
      if (row?.bonus)
        yield {
          kind: `progression:${cls}`,
          name: `${cls} L${lvl} bonus`,
          text: row.bonus,
          progression: { cls, lvl: +lvl },
        };
    }
  }
...
for (const a of abilities()) {
  ...
  for (const { cat, re } of SIGNALS) {
    ...
    const formExcluded =
      (cat === "maxLifePoints" || cat === "armor" || cat === "maxSpikes") &&
    ...
    if (caught === "graph") {
      ...
      gaps.push({
        kind: a.kind,
        name: a.name,
        cat: `${cat} (unscanned source field)`,
        sentence: firstEffectSentence(a.text, cat),
      });
