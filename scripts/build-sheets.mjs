...
const TIER_FIELD = {
  ...
  classSkills: "classPowers",
  rightHandPowers: "rightHandPowers",
  cantrips: "cantrips",
  noviceSpells: "noviceSpells",
  adeptSpells: "adeptSpells",
  greaterSpells: "greaterSpells",
};
...
function buildStrongest(name, classLevels) {
  ...
  for (const slot of r.slots) {
    ...
    for (const p of poolByClass[slot.cls] || []) {
      ...
      const ok =
        slot.category === "spellsKnown"
          ? /spell/.test(t)
          : slot.category === "cantrips"
            ? /cantrip/.test(t)
...
function layerPerksAndLineage(character) {
  ...
  for (const [name, lin] of Object.entries(LINEAGES)) {
    ...
    for (const sub of subs) {
      const inScope = (a) => {
        const k = subKey(a.sublineage);
        return k === "general" || k === sub;
      };
  ...
  if (best && best.val > 0) {
    const inScope = (x) => {
      const k = subKey(x.sublineage);
      return k === "general" || k === best.sub;
    };
    ...
    const advs = (best.lin.advantages || [])
      .filter(inScope)
    ...
    for (const { a } of advs) {
      ...
      const r = validate(character);
      if (!r.lbp || r.lbp.overspent || r.lbp.mixedSublineage || r.lbp.missingRequired?.length)
        character.lineageAdvantages.pop();
    }
  }
  ...
  const perkPool = ALL_PERKS.map((p) => ({ name: p.name, cost: p.cost || 0, v: entityValue(`perks:${p.name}`) }))
    .filter((p) => p.v > 0)
    .sort((a, b) => b.v - a.v);
