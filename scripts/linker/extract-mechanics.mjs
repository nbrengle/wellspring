...
const entities = [];
const add = (type, name, desc) => { if (name && desc) entities.push({ type, name, desc: String(desc) }); };
...
const CATS = [
  ['stat_mod', 'permanent stat mod (LP / Armor / Spike / Health / Natural Armor)',
...
if (auditStats) {
  ...
  const STRONG = new RegExp(
    ...
    'i');
  const STAT_OF = (snip) => /armor/i.test(snip) ? 'armor' : /spike/i.test(snip) ? 'spikes' : /natural/i.test(snip) ? 'naturalArmor' : 'lifePoints';

  const strong = entities
    .map((e) => ({ ...e, m: e.desc.match(STRONG) }))
    .filter((e) => e.m);

  console.log('═══ STAT-MOD CROSS-CHECK (prose states a max-stat mod → does the rail apply it?) ═══\n');
  const applied = [], missed = [], unchecked = [];
  for (const e of strong) {
    const snip = e.m[0];
    const expectStat = STAT_OF(snip);
    const char = charForEntity(e.type, e.name);
    if (!char) { unchecked.push({ ...e, snip }); continue; }
  ...
  console.log(`Strong stat-mod statements: ${strong.length}  (applied ${applied.length}, missed ${missed.length}, unchecked ${unchecked.length})\n`);
  if (missed.length) {
    console.log('⚠ STATED IN PROSE BUT NOT APPLIED BY THE RAIL — candidate bugs:');
    for (const e of missed) console.log(`   [${e.type}] ${e.name}  (expect ${e.expectStat})\n        “${e.snip.trim()}”`);
  }
  if (unchecked.length) {
