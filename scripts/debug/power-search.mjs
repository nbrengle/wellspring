...
function scoreLoadout(powers, pairs) {
  ...
  for (const p of powers)
    for (const h of p.hits) byEffect[h.name] = (byEffect[h.name] || 0) + h.w * freqMult(p.refresh);
  ...
  for (const [k, n] of pairs) {
    const [a, b] = k.split("|");
    if (have.has(a) && have.has(b)) synergy += Math.min(n, 5);
  }
...
function bestLoadout(classLevels) {
  ...
  for (const slot of r.slots) {
    ...
    for (const p of pool) {
      ...
      const ok = cat === "spellsKnown" ? /spell/.test(t) : cat === "cantrips" ? /cantrip/.test(t) : t.includes(cat);
      if (!ok) continue;
      picked.push(p);
      n--;
    }
...
const BASE_CLASSES = Object.keys(CLASS_POWERS).filter(
  (c) => CLASS_POWERS[c].innate || CLASS_POWERS[c].utility || CLASS_POWERS[c].basic || CLASS_POWERS[c].cantrips,
);
...
const EFFECT_ROLE = {
  ...
  Resist: "survivability",
  Counter: "survivability",
  Immunity: "survivability",
  Cure: "sustain/cleanse",
  Heal: "sustain",
  Revive: "sustain",
  Grant: "support/buff",
  "Grant Power": "support/buff",
  Imbue: "support/buff",
  Death: "offense",
  Wounding: "offense",
  Piercing: "offense",
...
function rationale(b) {
  ...
  const spammers = b.picked.filter((p) => p.hits.some((h) => h.name === b.topEffect)).sort((x, y) => y.score - x.score);
  const spamLine = spammers
    .slice(0, 3)
    .map((p) => `${p.name} (${REFRESH_NOTE(p.refresh)})`)
    .join(", ");
  ...
  const top = b.picked
    ...
    .slice(0, 4)
    .map((p) => `${p.name} [${p.topEffects.slice(0, 3).join("/")}]`)
    .join("; ");
  ...
  const lines = [];
  lines.push(
    `Role: **${role}**. Concentrates on **${b.topEffect}** (score ${b.concentration}) — delivered by ${spamLine || "multiple powers"}, so it can apply that effect repeatedly rather than once per event.`,
  );
  lines.push(`Heaviest powers: ${top}.`);
  lines.push(
    `Breadth: fields ${distinctEffects} distinct effects (synergy ${b.synergy}) — ${b.synergy > 250 ? "a wide, flexible toolkit" : b.synergy > 150 ? "solid cross-coverage" : "focused rather than broad"}.`,
  );
  return lines;
}
...
const singles = BASE_CLASSES.map((cls) => bestLoadout(`${cls} 10`))
  .filter(Boolean)
...
for (const s of singles.slice(0, 6)) {
  console.log(
    `  ${s.classLevels.padEnd(14)} power ${String(s.total).padStart(3)} | spams "${s.topEffect}" (${s.concentration}) | synergy ${s.synergy}`,
  );
}
console.log("");
...
for (let i = 0; i < BASE_CLASSES.length; i++)
  for (let j = i + 1; j < BASE_CLASSES.length; j++) {
...
for (const s of pairBuilds.slice(0, 6)) {
  console.log(
    `  ${s.classLevels.padEnd(22)} power ${String(s.total).padStart(3)} | spams "${s.topEffect}" (${s.concentration}) | synergy ${s.synergy}`,
  );
}
console.log("");

// 3. The single strongest build, detailed.
const champ = [...singles, ...pairBuilds].sort(
  (a, b) => b.total + b.concentration + b.synergy - (a.total + a.concentration + a.synergy),
)[0];
console.log(
  `★ Strongest overall: ${champ.classLevels}  (total ${champ.total}, concentration ${champ.concentration} on "${champ.topEffect}", synergy ${champ.synergy})`,
);
console.log("  loadout:");
for (const p of champ.picked.sort((a, b) => b.score - a.score).slice(0, 10)) {
  console.log(
    `    ${p.name.padEnd(26)} [${p.tier}/${p.refresh}]  effects: ${p.topEffects.join(", ")}  (score ${Math.round(p.score)})`,
  );
}
