...
const RELATION_MAP = {
  ...
  causedBy:       { enforcer: '(in-play effect — out of builder scope)',  countBy: 'keys', kind: 'in-play', mirror: 'causesCondition' },
...
console.log(`Entries WITH an enforcing code path: ${enforcedEntries} / ${totalRuleEntries}` +
...
if (unenforced.length) {
  ...
  for (const r of unenforced) {
    console.log(`   • ${r.rel} — ${r.entries} entries, NO validator reads this` +
...
for (const r of rows.filter((r) => r.mirror)) {
  ...
  const baseComparable = new Set(
    r.mirror === 'prereqs'
      ? [...base]
      : [...base].map((p) => { const [a, b] = p.split('>>'); return `${b}>>${a}`; })
  );
  const unmatched = [...fwd].filter((p) => !baseComparable.has(p));
  const ok = unmatched.length === 0;
  console.log(`   ${ok ? '✓' : '✗ NOT A CLEAN MIRROR'}  ${r.rel} = transpose(${r.mirror})` +
...
const fnNames = new Set(
  [...validateSrc.matchAll(/(?:export\s+)?function\s+([a-zA-Z][a-zA-Z0-9]*)/g)].map((m) => m[1])
);
