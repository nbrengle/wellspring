...
(migrate it to V2)** — do **not** type the V1 shape. *Sooner remove V1 than make it
pass.* Rip up whatever's needed; nothing about the current shapes is sacred.
...
- `tsc --noEmit`: **183 errors** (down from 208). All pre-existing untyped-TS debt;
  the migration work has only *reduced* it, never added.
...
## The core problem still to solve: V1 is still the *native* shape
...
`v1ToV2()`. So V2 only lives *inside* the engine.
...
- `v1ToV2()` + `V1CharacterInput` (src/engine/graph.ts, types.ts) — the bridge; the
  *goal is to delete these entirely*.
...
- **Build keys, don't parse them.** No `id.slice(0, indexOf(':'))` /
  `.split(':')[0]` to *recover* a type — read `.type`/`.name` off the object.
...
- **Grants carry params; cap/dedupe is `reusable = cap > |paramDomain|`** — see
  `DEDUPE_IDENTITY_PLAN.md`. Over-cap *purchase* = validation error; over-cap
  *grant* = free BP.
